...
export default function AwardsList({ awards }: { awards: Award[] }) {
  if (awards.length === 0) {
    return (
      <div style={{
        ...
        <div style={{
          width: 48, height: 48, border: "1px solid var(--border-faint)",
          display: "flex", alignItems: "center", justifyContent: "center", marginBottom: "1.5rem",
        }}>
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="var(--text-4)" strokeWidth={1.5}>
        ...
        <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "1.4rem", fontWeight: 300, color: "var(--text-4)", marginBottom: "0.5rem" }}>
          No awards published yet
        </p>
        <p style={{ fontSize: "0.75rem", color: "var(--text-5)", letterSpacing: "0.04em" }}>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "3rem" }}>
        ...
        <div key={year}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "1.25rem", marginBottom: "1.25rem" }}>
            <span style={{
              ...
              color: "var(--text-5)", letterSpacing: "0.08em",
              flexShrink: 0,
            }}>
              {year}
            </span>
            <div style={{ flex: 1, height: 1, background: "var(--border-faint)" }} />
            <span style={{ fontSize: "0.6rem", fontWeight: 600, letterSpacing: "0.2em", textTransform: "uppercase", color: "var(--text-5)", flexShrink: 0 }}>
              {grouped[year].length} {grouped[year].length === 1 ? "Category" : "Categories"}
            </span>
          </div>

          {/* Award cards — gapless grid with 1px separators */}
          <div style={{ border: "1px solid var(--border-faint)", background: "var(--surface-subtle)", display: "flex", flexDirection: "column" }}>
            {grouped[year].map((award, i) => (
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  {award.description && (
                    <p style={{
                      fontSize: "0.85rem", color: "var(--text-4)",
                      lineHeight: 1.7,
                      display: "-webkit-box",
                      WebkitLineClamp: 2,
      ...
      <style>{`
        .award-row:hover { background: var(--surface-hover) !important; }
      `}</style>
    </div>
  );
}
